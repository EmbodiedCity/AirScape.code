...
from finetune.data_modules import I2VDatasetWithResize, T2VDatasetWithResize
...
class Trainer:
    ...
    def prepare_trackers(self) -> None:
        ...
        # 过滤配置，只保留TensorBoard支持的数据类型
        config_dict = self.args.model_dump()
        filtered_config = {}
        for k, v in config_dict.items():
            if isinstance(v, (int, float, str, bool)) and v is not None:
                ...
                filtered_config[k] = str(v)  # 转换为字符串
        
        # 使用过滤后的配置初始化跟踪器
        self.accelerator.init_trackers(tracker_name, config=filtered_config)
